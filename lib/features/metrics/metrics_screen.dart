...
class _MetricsScreenState extends State<MetricsScreen> {
  ...
  void initState() {
    ...
    _metaControllers =
        List.generate(12, (i) => TextEditingController(text: metas[i]));

    _cobradasControllers = List.generate(
        12,
        (i) => TextEditingController(
            text: _calcularHorasPorTipo(i + 1, 'Hs Cobradas')));
    _investimentoControllers = List.generate(
        12,
        (i) => TextEditingController(
            text: _calcularHorasPorTipo(i + 1, 'Hs Investimento')));
    _naoCobradasControllers = List.generate(
        12,
        (i) => TextEditingController(
            text: _calcularHorasPorTipo(i + 1, 'Hs Não cobradas')));
    _internasControllers = List.generate(
        12,
        (i) => TextEditingController(
            text: _calcularHorasPorTipo(i + 1, 'Hs Internas')));
    _outrasControllers = List.generate(
        12,
        (i) => TextEditingController(
            text: _calcularHorasPorTipo(i + 1, 'Outras')));
    _naoInformadasControllers = List.generate(
        12,
        (i) => TextEditingController(
            text: _calcularHorasPorTipo(i + 1, 'Hs Não Informadas')));

    widget.timeLogStore.addListener(_atualizarValoresComLogs);
  }

  @override
  void dispose() {
    widget.timeLogStore.removeListener(_atualizarValoresComLogs);
  ...
  void _atualizarValoresComLogs() {
    ...
    setState(() {
      for (int i = 1; i <= 12; i++) {
        _cobradasControllers[i - 1].text =
            _calcularHorasPorTipo(i, 'Hs Cobradas');
        _investimentoControllers[i - 1].text =
            _calcularHorasPorTipo(i, 'Hs Investimento');
        _naoCobradasControllers[i - 1].text =
            _calcularHorasPorTipo(i, 'Hs Não cobradas');
        _internasControllers[i - 1].text =
            _calcularHorasPorTipo(i, 'Hs Internas');
        _outrasControllers[i - 1].text = _calcularHorasPorTipo(i, 'Outras');
        _naoInformadasControllers[i - 1].text =
            _calcularHorasPorTipo(i, 'Hs Não Informadas');
      }
    });
  }

  String _calcularHorasPorTipo(int mes, String tipoHs) {
    ...
    for (var log in widget.timeLogStore.logs) {
      if (log.date.month == mes && log.date.year == 2026) {
        ...
        if (match) {
          totalHoras += _parseTimeToDouble(log.durationFormatted);
        }
      }
    }
  ...
  void _recalcularDiasUteis() {
    ...
    try {
      ...
      if (parts.length == 3) ano = int.parse(parts[2]);
    } catch (_) {}
  ...
  int _calcularDiasUteis(int year, int month) {
  ...
  double _getRealizadoSum(int index) {
    return _parseTimeToDouble(_cobradasControllers[index].text) +
        _parseTimeToDouble(_investimentoControllers[index].text) +
        _parseTimeToDouble(_naoCobradasControllers[index].text) +
        _parseTimeToDouble(_internasControllers[index].text) +
        _parseTimeToDouble(_outrasControllers[index].text) +
        _parseTimeToDouble(_naoInformadasControllers[index].text);
  }
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          _buildLegendItem('Meta', _metaColor),
                          const SizedBox(width: 20),
                          _buildLegendItem('Realizado', _realizadoColor),
                        ],
                      ),
                    ],
                  ),
                        ...
                        return Row(
                          ...
                          children: [
                            ...
                            ...List.generate(12, (index) {
                              ...
                              final realizadoVal = _getRealizadoSum(index);
                              final realizadoText =
                                  _formatDoubleToTime(realizadoVal);

                              final double metaH =
                                  (metaVal / maxValor) * availableHeight;
                              final double realizadoH =
                                  (realizadoVal / maxValor) * availableHeight;
                ...
                children: [
                  TableRow(
                    ...
                    children: [
                      ...
                      _buildCellInput(_horasDiaController,
                          isHeader: true, bgColor: const Color(0xFFFFF2CC)),
                      ...List.generate(
                          12,
                          (i) => _buildCellText(_meses[i],
                              isHeader: true, color: Colors.white)),
                    ],
                  ),
                  TableRow(
                    ...
                    children: [
                      ...
                      _buildCellText('', isHeader: true),
                      ...List.generate(
                          12,
                          (i) => _buildCellText('${i + 1}',
                              isHeader: true, color: Colors.white)),
                    ],
                  ),
                  TableRow(
                    children: [
                      _buildCellText('Dias Úteis', alignLeft: true),
                      _buildCellText(
                          '${_diasUteisCalculados.fold<int>(0, (sum, val) => sum + val)}'),
                      ...List.generate(12,
                          (i) => _buildCellText('${_diasUteisCalculados[i]}')),
                    ],
                  ),
                  TableRow(
                    decoration: const BoxDecoration(color: Color(0xFFFFF2CC)),
                    children: [
                      _buildCellText('Meta', alignLeft: true, isBold: true),
                      _buildCellText(
                          _formatDoubleToTime(_metaControllers.fold<double>(
                              0,
                              (sum, item) =>
                                  sum + _parseTimeToDouble(item.text))),
                          isBold: true),
                      ...List.generate(
                          12,
                          (i) => _buildCellInput(_metaControllers[i],
                              isBold: true)),
                    ],
                  ),
                  TableRow(
                    children: [
                      _buildCellText('Hs Totais Realizadas',
                          alignLeft: true, isBold: true),
                      _buildCellText(
                          _formatDoubleToTime(
                              List.generate(12, (i) => _getRealizadoSum(i))
                                  .reduce((a, b) => a + b)),
                          isBold: true),
                      ...List.generate(
                          12,
                          (i) => _buildCellText(
                              _formatDoubleToTime(_getRealizadoSum(i)),
                              isBold: true)),
                    ],
                  ),
                  TableRow(
                    children: [
                      _buildCellText('Hs Cobradas', alignLeft: true),
                      _buildCellText(_formatDoubleToTime(
                          _cobradasControllers.fold<double>(
                              0,
                              (sum, item) =>
                                  sum + _parseTimeToDouble(item.text)))),
                      ...List.generate(
                          12, (i) => _buildCellInput(_cobradasControllers[i])),
                    ],
                  ),
                  TableRow(
                    children: [
                      _buildCellText('Hs Investimento', alignLeft: true),
                      _buildCellText(_formatDoubleToTime(
                          _investimentoControllers.fold<double>(
                              ...
                              (sum, item) =>
                                  sum + _parseTimeToDouble(item.text)))),
                      ...List.generate(12,
                          (i) => _buildCellInput(_investimentoControllers[i])),
                    ],
                  ),
                  TableRow(
                    children: [
                      _buildCellText('Hs Não cobradas', alignLeft: true),
                      _buildCellText(_formatDoubleToTime(
                          _naoCobradasControllers.fold<double>(
                              ...
                              (sum, item) =>
                                  sum + _parseTimeToDouble(item.text)))),
                      ...List.generate(12,
                          (i) => _buildCellInput(_naoCobradasControllers[i])),
                    ],
                  ),
                  TableRow(
                    children: [
                      _buildCellText('Hs Internas', alignLeft: true),
                      _buildCellText(_formatDoubleToTime(
                          _internasControllers.fold<double>(
                              ...
                              (sum, item) =>
                                  sum + _parseTimeToDouble(item.text)))),
                      ...List.generate(
                          12, (i) => _buildCellInput(_internasControllers[i])),
                    ],
                  ),
                  TableRow(
                    children: [
                      _buildCellText('Outras', alignLeft: true),
                      _buildCellText(_formatDoubleToTime(
                          _outrasControllers.fold<double>(
                              ...
                              (sum, item) =>
                                  sum + _parseTimeToDouble(item.text)))),
                      ...List.generate(
                          12, (i) => _buildCellInput(_outrasControllers[i])),
                    ],
                  ),
                  TableRow(
                    children: [
                      _buildCellText('Hs Não Informadas', alignLeft: true),
                      _buildCellText(_formatDoubleToTime(
                          _naoInformadasControllers.fold<double>(
                              ...
                              (sum, item) =>
                                  sum + _parseTimeToDouble(item.text)))),
                      ...List.generate(12,
                          (i) => _buildCellInput(_naoInformadasControllers[i])),
                    ],
                  ),
                ],
              ),
  ...
  Widget _buildCellInput(TextEditingController controller,
      {bool isHeader = false,
      Color? bgColor,
      bool isBold = false,
